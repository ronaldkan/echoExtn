...
socket.on('connect', function () {
	...
	var roomLocation = window.location.pathname.substring(1);
	var params = {
		room: window.location.host + ' - ' + (roomLocation ? roomLocation : 'Home'),
		name: currentUserName
	};

	var roomName = jQuery('<h3></h3>').text(params.room);
	var userName = jQuery('<p></p>').text('You are ' + currentUserName);
...
socket.on('newMessage', function (message) {
	...
	if (message.from === 'Admin') {
		template = jQuery('#admin-message-template').html();
		...
		if (message.from === currentUserName) {
			template = jQuery('#user-message-template').html();
		}
		html = Mustache.render(template, {
			text: message.text,
			from: message.from,
...
socket.on('updateUserList', function (users) {
	var p = jQuery('<p></p>').text(users.length + ' Online');
	jQuery('#users').html(p);
})
